            ...
            switch (currentState) {
                ...
                case DRAW_SECOND_FILTERED_IMAGE:
					drawImage(FULLSCREEN_H_SIZE, FULLSCREEN_V_SIZE, IMAGE_H_SIZE, IMAGE_V_SIZE,
							  ...
							  1, filteredImage, Buffer);
					clockCycles = cyclesForDraw;
                    if (active_connections > 0 && data_received != 0) {
...
int transfer_data(struct tcp_pcb *tpcb) {
    ...
    if (space_in_tcp_buffer == 0) {
        buffer_full_timeout++;
        if (buffer_full_timeout > max_buffer_full_cycles) {
            xil_printf("Erro ao transferir, buffer cheio.\n");
            transfering = false;
            send_index = 0;
            buffer_full_timeout = 0;
        }
        return 1;
    }
    buffer_full_timeout = 0;
...
err_t recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    if (!p) {
        active_connections--;
        xil_printf("Conexão encerrada. Conexões ativas: %d\n", active_connections);
...
err_t accept_callback(void *arg, struct tcp_pcb *newpcb, err_t err)
{
    ...
    active_connections++;  // Incrementa o contador de conexões ativas
    xil_printf("Nova conexao aceita. Conexoes ativas: %d\n", active_connections);
